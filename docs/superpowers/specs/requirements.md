...
| 子需求 | 描述 | 设计参考 |
|--------|------|----------|
| FR-4.1 | 添加机器 | 见 [Design Spec](./2026-05-08-multi-agent-collab-design.md) §7.1 |
...
> 完整 Schema 见 [Design Spec](./2026-05-08-multi-agent-collab-design.md) §4
...
**认证：** 首次连接时发送 `machine_register` 消息

```typescript
// 客户端 → 服务器
interface ClientMessage {
  type: 'machine_register' | 'send_message' | 'join_channel' | 'leave_channel' | 'create_agent';
  // ... 根据 type 不同字段
}

// 服务器 → 客户端
interface ServerMessage {
  type: 'message' | 'presence' | 'agent_output' | 'agent_error' | 'registered';
  // ... 根据 type 不同字段
}
```

详细消息格式见 [Design Spec](./2026-05-08-multi-agent-collab-design.md) §5.1
...
| POST | /api/cards | { columnId, title } | Card | FR-7.2 |
| PATCH | /api/cards/:id | { columnId?, position? } | Card | FR-7.3 |
...
详见：[Design Spec](./2026-05-08-multi-agent-collab-design.md) §7.2
...
| T1.2 | SQLite 数据库设置 | - |
| T1.3 | REST API 实现 | T1.2 |
| T1.4 | WebSocket 服务器 | T1.3 |
...
- [Claude Code Help](./cli-agent.md) - Claude Code 命令行参数